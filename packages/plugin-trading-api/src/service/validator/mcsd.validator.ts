...
export default class TransactionMCSDValidator extends BaseValidator {
    ...
    validateSaveTransaction = async (params) => {
        var { error, data } = this.validate({
            BeginDate: Joi.date().default(new Date(new Date().getTime() - 24 * 60 * 60 * 1000)),
        ...
        let tomorrow = new Date(new Date(data.EndDate).getTime() + 24 * 60 * 60 * 1000);
    ...
    validateGet = async (params) => {
        var { error, data } = this.validate({
            ...
            transactionDate: Joi.object({ gt: Joi.date(), gte: Joi.date(), lt: Joi.date(), lte: Joi.date() }).required(),
        ...
        let transactions:any = await this._transactionMCSDRepository.findAll(data, undefined, options);
        let beginBalance = await this._transactionMCSDRepository._prisma.$queryRaw`SELECT IFNULL(sum(case when tr.transactionTypeId in (11,30,40,50,61) then (tr.securitiesQuantity) when tr.transactionTypeId in (10,20,41,51,52,53,60) then (tr.securitiesQuantity * -1) end),0) AS securitiesQuantity,tr.securitiesCode,tr.mitPrefix
