...
export default class Securities {
    ...
    GetOneTransaction = async (params) => {
        ...
        for (let i = 0; i < transactions.length; i++) {
            ...
            transaction.securitiesQuantity = parseFloat(transactions[i].SecuritiesQuantity);
            ...
            transaction.transactionTypeId = parseInt(transactions[i].TransactionTypeId);
        ...
        for (let i = 0; i < transactions.length; i++) {
            ...
            let stock = await this.stockRepository.getByStockcode(transaction.securitiesCode.toString());
            if (stock) {
                if(transaction.transactionTypeId == 10 || transaction.transactionTypeId == 11 || transaction.transactionTypeId == 40 || transaction.transactionTypeId == 41){
                    try{
                        let userWallet = await this.userRepository.findWithWalletByPrefix(transaction.mitPrefix.toString());
                    if(userWallet.length != 0){
                        let stockTran;
                        if(transaction.transactionTypeId == 11 || transaction.transactionTypeId == 40){
                        ...
                        }else if(transaction.transactionTypeId == 10 || transaction.transactionTypeId == 41){
                            stockTran = await this.stockTransactionService.w2w({
