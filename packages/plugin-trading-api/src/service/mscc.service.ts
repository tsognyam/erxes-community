...
export default class Clearing {
  ...
  GetPaymentClient = async data => {
    ...
    for (let i = 0; i < res.data.length; i++) {
      let user = await this.userRepository.findByPrefix(
        res.data[i].ClientPrefix.toString()
      );
      if (user.length != 0) {
  ...
  GetPaymentFee = async data => {
    ...
    for (let i = 0; i < res.data.length; i++) {
      let user = await this.userRepository.findWithWalletByPrefix(
        res.data[i].prefix.toString()
      );

      if (user.length != 0) {
        let orders = await this.orderRepository.getOrderList(
          user[0].id,
          Helper.dateToString(new Date(res.data[i].tradeDate), true)
        );
