...
export default class HttpService {
    ...
    async request(url, params = null, method = HttpService.METHOD_GET, headers = null) {

        let option;
        // axios.post(this._baseUrl, JSON.stringify(params))
        if (method == HttpService.METHOD_GET || method == HttpService.METHOD_DELETE) {
            option = {
                method: method,
        ...
        return await axios(option).catch((error) => {
            ...
            if (error.response) {
                ...
                throw new ErrorException(500, 'error API');
            } else {
